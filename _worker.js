...
// 删除用户并吊销其令牌
async function deleteUserAndRevokeToken(username) {
  const fullUserName = username.trim();
  const url = new URL(request.url);
  // 获取现有用户列表
  const Users = await KV.get('Users', { type: 'json' });
  const FreeUsers = await KV.get('FreeUsers', { type: 'json' });
  const VIPUsers = await KV.get('VIPUsers', { type: 'json' });

  // 过滤掉该用户
  const newUsers = Users.filter(user => user !== fullUserName);
  const newFreeUsers = FreeUsers.filter(user => user !== fullUserName);
  const newVIPUsers = VIPUsers.filter(user => user !== fullUserName);

  // 更新用户列表
  await KV.put('Users', JSON.stringify(newUsers));
  await KV.put('FreeUsers', JSON.stringify(newFreeUsers));
  await KV.put('VIPUsers', JSON.stringify(newVIPUsers));

  // 吊销用户的共享令牌 (假设共享令牌在 KV 存储中是以 `sharetoken-username` 形式存储的)
  await KV.delete(`sharetoken-${fullUserName}`);

  return `用户 ${fullUserName} 已被成功删除并吊销了令牌`;
}
...
if (cookies) {
  ...
  if (url.pathname === '/api/deleteUser' && request.method === 'POST') {
    try {
      // 解析传入的请求体
      ...
      // 调用删除用户和吊销令牌的逻辑
      const result = await deleteUserAndRevokeToken(username);
      return new Response(result, { status: 200 });
    } catch (error) {
      return new Response(`请求处理出错: ${error.message}`, { status: 500 });
    }
  } else {
    return new Response('未找到此 API 路径', { status: 404 });
  }
   ...
   const commonHTML2 = `
       ...
       document.getElementById('cf-turnstile-response').value= "111";
             ...
             document.addEventListener('DOMContentLoaded', function() {
                 ...
                 tooltip.textContent = '每一个序号都代表一个Plus账号，当账号次数限制或者出现异常时可以尝试登录不同账号继续使用，不同账号间对话隔离，别人无法看见你的对话';
